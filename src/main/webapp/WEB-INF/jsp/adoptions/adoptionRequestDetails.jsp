...
<petclinic:layout pageName="adoptionDetails">
	<table id="adoptionDetails" class="table table-striped">
		...
		<tr>
			...
			<td>${request.suitorToAdopt.user.username}</td>
		</tr>
		<tr>
			<th style="width: 200px">Dirección</th>
			<td>${request.suitorToAdopt.address}</td>
		</tr>
		<tr>
			<th style="width: 120px">Cuidad</th>
			<td>${request.suitorToAdopt.city}</td>
		</tr>
		<tr>
			<th style="width: 120px">Teléfono</th>
			<td>${request.suitorToAdopt.telephone}</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>${request.careDescription}</td>
		</tr>
	</table>

	<spring:url
		value="/adoption/{adoptionId}/suitor/{suitorId}/approve"
		var="aprobacionURL"
	>
		<spring:param name="adoptionId" value="${request.adoption.id}" />
		<spring:param name="suitorId" value="${request.suitorToAdopt.id}" />
	</spring:url>
	<a href="${fn:escapeXml(aprobacionURL)}" class="btn btn-default"
		><c:out value="Aprobar adopción"
	/></a>
</petclinic:layout>
